...
class FeedbackRecord:
    ...
    # 2026-05-21 reproducibility fields — symmetric with AuditEvent.
...
def build_feedback_record(
    ...
    return FeedbackRecord(
        ...
        now_anchor_iso=pipeline_result.get("now_anchor_iso") or "",
        knowledge_git_sha=pipeline_result.get("knowledge_git_sha") or "",
        permitted_tables=list(pipeline_result.get("permitted_tables") or []),
        platform_context=pipeline_result.get("platform_context") or "",
        retry_context=pipeline_result.get("retry_context") or "",
        tool_calls_log=list(pipeline_result.get("tool_calls_log") or []),
        explain_plan=dict(pipeline_result.get("explain_plan") or {}),
    )
